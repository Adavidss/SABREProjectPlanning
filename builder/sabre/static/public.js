...
// Workstreams are the workbook's own sections. "Planned" reads solid (expected-window) fills for the current week; pale fills are pessimistic extensions.
function roadmapStreams(){
 ...
 for(const row of r.rows){
  if(row.section){const parts=row.section.split(/\s{2,}/)[0].split(' - ');s={name:parts.shift().trim(),detail:parts.join(' - ').trim(),rows:[],color:''};out.push(s);continue;}
...
function workstreamStrip(){
 const statements=(data.overview.program||[]).filter(x=>x.kind==='workstream'),norm=v=>String(v||'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();
 const {week,streams}=data.overview.display?.show_roadmap===false?{week:-1,streams:[]}:roadmapStreams(),used=new Set();
 ...
 const items=streams.map(s=>{const st=statements.find(x=>{const a=norm(x.title),b=norm(s.name);return a&&b&&(a===b||a.startsWith(b)||b.startsWith(a));});if(st)used.add(st);
  const plan=week<0?'':s.now.length?`<p class="stream-plan"><span class="plan-label">Planned this week</span>${itemButton('data-open-roadmap',s.now[0].id,s.now[0].title,'open in the Gantt')}${s.now.length>1?`<small>+${s.now.length-1} more planned</small>`:''}</p>`:`<p class="stream-plan quiet"><span class="plan-label">Planned this week</span><span>Nothing in the expected window${s.next?` · next: ${e(s.next.title)}, week of ${e(data.roadmap.weeks[s.next.first].start)}`:''}</span></p>`;
  return `<li class="stream"><span class="stream-swatch" style="background:${e(s.color)}" aria-hidden="true"></span><div><h3>${e(s.name)}</h3>${s.detail?`<small class="stream-detail">${e(s.detail)}</small>`:''}${statement(st)}${plan}</div></li>`;});
 statements.filter(x=>!used.has(x)).forEach(x=>items.push(`<li class="stream"><span class="stream-swatch" aria-hidden="true"></span><div><h3>${e(x.title)}</h3>${statement(x)}</div></li>`));
...
function programSummary(){
 ...
 return `<section class="program-orientation summary-panel" aria-label="Program orientation">${phase?`<p class="phase-line"><span>Current phase</span> ${e(phase.text)} ${attribution(phase)}</p>`:''}<div class="direction-grid"><div class="summary-tile focus-tile${focus?'':' is-empty'}"><h2>${icon('target')}Current focus</h2>${focus?`<p class="summary-statement">${e(focus.text)}</p>${attribution(focus)}`:'<p class="empty">No current focus statement published.</p><small class="empty-hint">The project owner publishes focus statements with a date and source.</small>'}</div>${showMilestones?`<div class="summary-tile milestone-tile"><h2>${icon('diamond')}Next scheduled milestone</h2>${next?`<p class="summary-statement">${itemButton('data-open-roadmap',next.id,next.title,'open in the Gantt')}</p><small>Week of ${e(next.date)}${weeks>0?` · in about ${weeks} week${weeks===1?'':'s'}`:''} · original Gantt, planned</small>`:'<p class="empty">No future milestone in the source schedule.</p>'}</div>`:''}</div>${workstreamStrip()}</section>`;
}
function roadmapSummary(){
 const milestones=roadmapMilestones().filter(x=>!x.completed&&x.date>=data.today).slice(0,4);
 return data.overview.display?.show_milestones===false?'':`<ol class="roadmap-summary milestone-timeline">${milestones.map(x=>`<li><span class="timeline-node" aria-hidden="true"></span><time>Week of ${e(x.date)}</time>${x.kind==='Milestone'?'':`<span class="kind-chip">${e(x.kind)}</span>`}${itemButton('data-open-roadmap',x.id,x.title,'open in the Gantt')}</li>`).join('')||'<li class="empty">No upcoming milestones recorded in the workbook.</li>'}</ol><small class="caption">Planned dates are not evidence of completion.</small>`;
}
